...
1:
	...
	tio.bseti RCC_CFGR, RCC_CFGR, RCC_CFGR_SW_PLL
...
void init_clocks2()
{
	FLASH->ACR |= FLASH_ACR_PRFTBE_Msk | (FLASH_ACR_LATENCY_Msk & 0b001); // ws is 0 or 1
...
NOTE: gcc fails to detect `bfi` from constant pattern generally (clang exists in armv7-a version only)
