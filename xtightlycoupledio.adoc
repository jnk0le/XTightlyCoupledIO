...
- otherwise the content is used in/comes from basic ALU computations
- register content is immediately converted to float for computation in control loop
...
due to requirement of atomicity when changing 16bit stack pointer, which is mapped to IO address space.
...
Source and destination operands can independently address their bytes and half-words.
...
NOTE: even though it's seems to be the simplest approach, compilers are not allowing
direct usage of upper registers in RV32E 

==== use custom `csr` registers

csrr* instructions implement an atomic swap and immediate bitmask set/clear operations.+
Only bottom 5 bits are reachable by immediate.
...
This exact approach has been implemented in risc-v based esp32 <<esp32c6riscv>>, allows only
for 8 GPIO pins mapped into csr registers (separately for input and output). 
Reads incur extra 2 cycle penalty.
...
==== use reserved registers

by ABI deviations or `-ffixed-xN` flag.
...
Which avoids introducing jitter/latency from a branch overhead.
