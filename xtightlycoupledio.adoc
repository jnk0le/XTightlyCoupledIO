...
In order to reduce phase erosion (by up to 18 degres according to <<an5305>>) the
ADC blanking period have to be extended towards the end of cycle. +
...
the explicit wait is necessary before reading result (`while(!(ADC1->ISR & ADC_ISR_JEOSMP));` which can
resolve to `1: tio.bsbclri ADC1_ISR, ADC_ISR_JEOSMP_Pos, 1b`)
- Use transposed direct form II with natural result latency of 1 MAC operation
- Direct form I implementation can precompute most of the accumulations in previous cycle
- Defer the state updates to after the write to timer registers (straightforward direct form I)
...
==== irq implementation (floating point)

3p3z control loop irq implemented with transpoed direct form II::
[source, C++]
----
#include <algorithm>
...
void ADC1_IRQHandler()
{
	...
	float yn = buck2.Z[0] + in * buck2.b[0];

	// apply scaling
	float out = yn * buck2.k;
	...
	buck2.Z[0] = buck2.Z[1] + in * buck2.b[1] + yn * buck2.a[1 - 1];
	buck2.Z[1] = buck2.Z[2] + in * buck2.b[2] + yn * buck2.a[2 - 1];
	buck2.Z[2] = in * buck2.b[3] + yn * buck2.a[3 - 1];
}
----
