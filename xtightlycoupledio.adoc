...
interrupt handler with 3p3z control loop::
[source, C++]
----
#include <algorithm>
...
typedef struct {
    ...
    // in `f.li` or `lui` (Zfinx) instructions, but that's a lot of manual work 
    float b[4];
...
=== stm32 ultra low latency 3p3z (FP)
...
=== stm32 buck converter (3p3z, voltage mode, fixed point)
