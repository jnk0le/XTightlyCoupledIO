...
=== stm32g4 buck converter irq (3p3z, voltage mode, floating point)
...
- timer saturates the output to maximum duty (no neg to zero saturation still)
- early conversion trigger not available

NOTE: On stm32g4 it is possible to do c2000 like early ADC trigger but
...
typical interrupt handler with 3p3z control loop::
[source, C++]
----
#include <algorithm>
...
typedef struct {
	// delay lines
	float X[3]; // input // -1 indexed, as X0 is handled on the fly
	float Y[3]; // output // -1 indexed, as Y0 is handled on the fly
...
void ADC1_IRQHandler()
{
	...
	// numerators
	float acc = in * buck1.b[0];
	acc += buck1.X[1 - 1] * buck1.b[1];
	acc += buck1.X[2 - 1] * buck1.b[2];
	acc += buck1.X[3 - 1] * buck1.b[3];

	// denominators
	acc += buck1.Y[1 - 1] * buck1.a[1 - 1];
	acc += buck1.Y[2 - 1] * buck1.a[2 - 1];
	acc += buck1.Y[3 - 1] * buck1.a[3 - 1];

	// apply scaling
	float out = acc * buck1.k;

	// saturate negative
	int sat = std::max((int)out, 0);

	// timer saturates positive values
	HRTIM1_TIMA->CMP1xR = sat;
	...
	// increases register pressure though 
	// compilers sometimes undefer it (arm-gcc at -O2)
	buck1.X[3 - 1] = buck1.X[2 - 1];
	buck1.X[2 - 1] = buck1.X[1 - 1];
	buck1.X[1 - 1] = in;

	buck1.Y[3 - 1] = buck1.Y[2 - 1];
	buck1.Y[2 - 1] = buck1.Y[1 - 1];
	buck1.Y[1 - 1] = acc;
}
----

To reduce register pressure and consequently improve code size (especially in case
of sw register stacking in irq handlers like CLIC), the X and Y arrays can be
updated immediately after elements being consumed (at least gcc can take some advantage of it)

NOTE: sometimes it makes no difference to compiler instruction scheduling

[source, C]
----
	// numerators
	float acc = buck1.X[3 - 1] * buck1.b[3];
	buck1.X[3 - 1] = buck1.X[2 - 1];
	acc += buck1.X[2 - 1] * buck1.b[2];
	buck1.X[2 - 1] = buck1.X[1 - 1];
	acc += buck1.X[1 - 1] * buck1.b[1];
...
ADC1_IRQHandler():                   # @ADC1_IRQHandler()
	...
	flw     fa1, 48(a2)
	flw     fa2, 52(a2)
	fmadd.s ft1, ft3, ft5, ft1
	fmadd.s ft1, ft6, ft7, ft1
	fmadd.s ft1, fa0, fa1, ft1
	fmul.s  ft5, ft1, fa2
	fcvt.w.s	a3, ft5, rtz
	max     a3, a3, zero
	lui     a4, 262167
...
NOTE: llvm likes to allocate a lot of registers, starting from `ft` ones of course.
GCC does a better job at reducing register pressure
...
NOTE: on CM4 those loads could be done by one big vldm. All FMAs are 3 cycle ones anyway

risc-v + XTightlyCoupledIO::
[source, asm]
----
	// if ADC did sign extension to whole 32 bits 
...
// NOTE: register pressure and code size analysis ommited due to compilers preffering 
// high pressure and codesize which biases the pure assembly implementation 

===== ultra low latency 3p3z

// assemby only
// compilers with interrupt attribute don't care
// for further see Xteic

//the accumulation can pre computed in previous cycle

//precompute and wait fotr EOC or precompute in previous cycle

===== Vref subtracted in software

in case of simple ADC
