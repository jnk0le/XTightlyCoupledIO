...
|====================================================================================
| Version | change
| v3.0.3  | add section bout MMIO less peripheral registers
...
Can use `bseti` or `bclri` to cover a single bit in a field (avoid loading constants).
...
It is possible to have SFRs that are not mapped to memory address space which are used by special 
`\\__attribute__\`, but this prevents use of a pointers to such peripherals.

Pointers are often used to avoid code duplication and resulting size increase <<moteusadcfix>>.
(even wrt. `tio` access, in some scenarios)

NOTE: still suitable for cases of a dedicated IO slave cores.
...
* [[[packetprocriscv, 3]]] https://liu.diva-portal.org/smash/get/diva2:1636414/FULLTEXT01.pdf
* [[[moteusadcfix, 4]]] https://github.com/mjbots/moteus/commit/a398d0c4fde08ea5a585bbf0d53da6be422e0915
