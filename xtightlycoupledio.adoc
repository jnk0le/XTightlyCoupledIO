...
24 bytes (22 if `DP` can be done by `MOVZ`)

NOTE: "fast branches" are slower when used on waitstated memories (like flash) <<c2000fastbranch>>.
The only slow branches are 32bit ones.
...
```
	...
	tio.bsbclri CpuTimer0_TCR, CpuTimer0_TCR_TIF_Pos, L1 // 12 bit range
	nop // placeholder
L1:
```
...
* [[[c2000fastbranch, 10]]] https://e2e.ti.com/support/tools/code-composer-studio-group/ccs/f/code-composer-studio-forum/447691/reason-for-not-using-c28x-fast-branch
